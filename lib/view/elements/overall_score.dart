...
class OverallScore extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (context.watch<GraphicalAbstractViewModel>().overallScore == 0) {
      return const SizedBox();
    } else {
      return Container(
        // decoration: BoxDecoration(color: UiColor.primaryColor.withOpacity(0.04), borderRadius: BorderRadius.circular(10)),
        ...
        child: Column(
          ...
          children: [
            ...
            Text("Your overall score is ${context.watch<GraphicalAbstractViewModel>().overallScore}/5", style: TextStyle(color: UiColor.extraDarkGreyColor, fontSize: 14, fontWeight: FontWeight.bold, fontFamily: 'Poppins')),
            const SizedBox(height: 20),
            SizedBox(
              width: 700,
              child: SfLinearGauge(
                ...
                interval: 0,
                maximum: 6,
                markerPointers: <LinearMarkerPointer>[LinearShapePointer(animationType: LinearAnimationType.ease, value: context.watch<GraphicalAbstractViewModel>().overallScore.toDouble() - 0.6, width: 20, height: 20, color: Colors.black, shapeType: LinearShapePointerType.triangle, position: LinearElementPosition.cross, markerAlignment: LinearMarkerAlignment.start)],
...
Widget labelBuilder(BuildContext context, String text) {
  return Center(
    child: Text(
      ...
      style: TextStyle(
        color: Colors.black,
        fontSize: ResponsiveHelpers.isMobile(context) ? 9 : 14,
        fontFamily: 'Poppins',
      ),
    ),
  );
